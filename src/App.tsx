...
import { FarmerProfile, authService } from "./services/authService";
...
export default function App() {
  ...
  // Phone OTP: state to store intermediate complete-profile redirect data
  const [completeProfileData, setCompleteProfileData] = useState<{ authUserId: string; phone: string } | null>(null);

  useEffect(() => {
    if (auth.user) {
      setIsLoggedIn(true);
      // Phone OTP users have phone instead of email — handle both
      const displayName =
        auth.user.user_metadata?.username ||
        auth.user.user_metadata?.full_name ||
        auth.farmerProfile?.name ||
        auth.user.phone ||
      ...
      const displayEmail =
        auth.user.email ||
        auth.user.phone ||
        auth.farmerProfile?.phone ||
        "";
      setUsernameState(displayName);
      setEmailState(displayEmail);
  ...
  }, [auth.user, auth.farmerProfile, currentPath]);
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      // 1. Process PKCE Code parameters (email/OAuth flows)
      if (code) {
        console.log("[Auth Callback] Detected 'code' parameter in URL. Commencing exchange...");
        try {
          // Use supabase directly since AuthContext now focuses on Phone OTP
          const { data, error } = await supabase.auth.exchangeCodeForSession(code);
          if (error) throw error;
          const session = data.session;
          console.log("[Auth Callback] Code exchange successful. User:", session?.user?.email);
          ...
          if (session?.user) {
            ...
            setEmailState(session.user.email || session.user.phone || "");
          }
  ...
  // Routes that require login before being accessed:
  const PROTECTED_PATHS = ["/dashboard", "/my-schemes", "/profile", "/community", "/saved-schemes", "/complete-profile"];
  const isProtectedPath = PROTECTED_PATHS.some(p => currentPath === p || currentPath.startsWith(p + "/"));

  // Show /complete-profile if user authenticated but has no farmer profile yet
  if (completeProfileData && (currentPath === "/complete-profile" || screen === "complete_profile")) {
    return (
      <CompleteProfilePage
        authUserId={completeProfileData.authUserId}
        userPhone={completeProfileData.phone}
        onProfileComplete={(profile: FarmerProfile) => {
          setCompleteProfileData(null);
          setIsLoggedIn(true);
          setUsernameState(profile.name || "Farmer");
          setEmailState(profile.phone || "");
  ...
  // If not logged in and on a protected path → show Mobile OTP Login
  if (!isLoggedIn && isProtectedPath) {
    return (
      <MobileOtpLogin
        onLoginSuccess={(farmerProfile, isNewUser) => {
          if (isNewUser || !farmerProfile) {
            // No profile: go to complete-profile
            if (auth.user) {
              setCompleteProfileData({ authUserId: auth.user.id, phone: auth.user.phone || "" });
            }
            navigateToPath("/complete-profile");
            setScreen("complete_profile");
          } else {
            setIsLoggedIn(true);
            setUsernameState(farmerProfile.name || "Farmer");
            setEmailState(farmerProfile.phone || "");
            navigateToPath("/dashboard");
            setScreen("dashboard");
          }
        }}
        onNavigateToCompleteProfile={(authUserId, phone) => {
          setCompleteProfileData({ authUserId, phone });
          navigateToPath("/complete-profile");
          setScreen("complete_profile");
        }}
      />
  ...
  // If not logged in and NOT on a protected path → show MobileOtpLogin only for /login
  if (!isLoggedIn && (currentPath === "/login" || screen === "login")) {
    return (
      <MobileOtpLogin
        onLoginSuccess={(farmerProfile, isNewUser) => {
          if (isNewUser || !farmerProfile) {
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-12 py-8 pb-32">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full"
          >
            {/* ── Phone OTP Auth Route: /complete-profile ── */}
            {(currentPath === "/complete-profile" || screen === "complete_profile") && completeProfileData ? (
              <CompleteProfilePage
                authUserId={completeProfileData.authUserId}
                userPhone={completeProfileData.phone}
                onProfileComplete={(profile: FarmerProfile) => {
              ...
              />
            ) : currentPath === "/dashboard" || screen === "dashboard" ? (
              /* /dashboard is protected — isLoggedIn guard already handled above the render */
              <FarmerDashboard
                user={null}
                onLogout={async () => {
                  await handleLogout();
                ...
                onNavigate={(path) => {
                  navigateToPath(path);
                  if (path === "/schemes") setScreen("schemes");
                  else if (path === "/mandi-prices") setScreen("mandi_prices");
                  else if (path === "/crops") setScreen("disease_detection");
                  else if (path === "/community") setScreen("community");
                  else if (path === "/profile") setScreen("farmer_profile");
                }}
              />
            ) : currentPath === "/profile" || screen === "farmer_profile" ? (
              /* /profile is protected — isLoggedIn guard already handled above */
              <FarmerProfilePage
                user={null}
                onProfileUpdated={(updated) => {
                  setUsernameState(updated.name || updated.mobileNumber || "Farmer");
                  setEmailState(updated.email || updated.mobileNumber || "");
                }}
                onBack={() => {
                  navigateToPath("/dashboard");
                  setScreen("dashboard");
            ...
            {/* ==================== 1. HOME DASHBOARD ==================== */}
            {(screen === "home" && currentPath !== "/login" && currentPath !== "/dashboard" && currentPath !== "/profile") && (
