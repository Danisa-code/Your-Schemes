...
export default function App() {
  ...
  const [quickFarmerName, setQuickFarmerName] = useState("");
  const [quickFarmerEmail, setQuickFarmerEmail] = useState("");
  const [showQuickForm, setShowQuickForm] = useState(false);

  const handleQuickSignIn = (e: React.FormEvent) => {
    e.preventDefault();
    if (!quickFarmerName.trim()) return;
    signInWithUser({
      sub: `farmer_${Date.now()}`,
      name: quickFarmerName.trim(),
      given_name: quickFarmerName.trim().split(" ")[0],
      email: quickFarmerEmail.trim() || `${quickFarmerName.trim().toLowerCase().replace(/\s+/g, "")}@farmer.portal`,
